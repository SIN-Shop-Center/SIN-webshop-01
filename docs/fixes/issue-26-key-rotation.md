...
In the current `main` branch, we already rotated the Stripe key (from `sk_live_...Y67wA` to `sk_live_...Y67wA`) and the Stripe key in `.env.local`/Cloudflare/GitHub Secrets. **However**, the OLD key is still visible in the git history.

**Confirmed in the file `STRIPE-KEY-ROTATION.md`:**
- Old key (`sk_live_...Y67wA`) — committed in earlier history
...
git filter-repo \
  --replace-text <(printf 'sk_live_...Y67wA\n<REDACTED>\n') \
  --force
```

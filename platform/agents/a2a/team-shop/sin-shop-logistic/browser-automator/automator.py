...
def main() -> int:
    ...
    if args.execute:
        parser.error("external execution is disabled; use the operator-approved browser workflow")
    ...
    (artifact_dir / "screenshot-manifest.json").write_text(
        json.dumps({"capture_on": audit["screenshot_checkpoints"], "captured": []}, indent=2),
        encoding="utf-8",
    )

...
export default async function TikTokAdminPage() {
  ...
  const [{ data: products }, { data: orders }, { data: auth }] = await Promise.all([
    supabase
      .from('products')
      .select('id, name, title_de, tiktok_status, tiktok_product_id, tiktok_last_error, tiktok_published_at')
      ...
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          {auth?.shop_id
            ...
            : 'Nicht verbunden — Seller-Autorisierung erforderlich.'}
        </p>
        {!auth?.shop_id ? (
          <div>
            <Link href="/api/tiktok/oauth/start" className="btn btn-primary btn-md inline-flex">
              TikTok Shop sicher verbinden
            </Link>
          </div>
        ) : null}
      </header>
            ...
            <tbody>
              {(products ?? []).map((p) => (
                <tr key={p.id} className="border-b border-border last:border-0">
                  <td className="p-3">{p.title_de || p.name}</td>

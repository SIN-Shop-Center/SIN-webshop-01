    ...
    try {
      ...
      if (!existing || existing.status === 'received' || existing.status === 'cj_failed') {
        if (!order.line_items.length) {
          throw new Error('TikTok-Order enthaelt keine Positionen')
        }

        const normalizedLines = order.line_items.map((lineItem) => ({
          lineItem,
          productId: lineItem.seller_sku.replace(/^SIN-/, ''),
        ...
        const cjProducts = normalizedLines.map(({ lineItem, productId, quantity }) => {
          const product = productsById.get(productId)
          if (!product?.cj_variant_id) {
            throw new Error(`Kein cj_variant_id fuer seller_sku ${lineItem.seller_sku}`)
          }
          return { vid: product.cj_variant_id, quantity }
        })
        ...
        await supabase.from('tiktok_orders').upsert({
          ...
          raw_order: JSON.parse(JSON.stringify(order)) as Json,

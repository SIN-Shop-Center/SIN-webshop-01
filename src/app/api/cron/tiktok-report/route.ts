...
export async function GET(request: Request) {
  ...
  for (const order of orders ?? []) {
    ...
    for (const li of raw?.line_items ?? []) {
      ...
      const rawQuantity = Number(li.quantity ?? 1)
      ...
      const { data: product } = await supabase
        .from('products')
        .select('cj_cost_price, name, title_de')
        .eq('id', productId)
        .maybeSingle()
      cjCost += Number(product?.cj_cost_price ?? 0) * quantity

      productSales[productId] = productSales[productId] ?? {
        title: product?.title_de ?? product?.name ?? li.product_name,
        count: 0,
        revenue: 0,
      }
  ...
  const topProducts = Object.values(productSales)
    ...
    .slice(0, 10)
    .map((p) => `<li>${escapeHtml(p.title)}: ${p.count}x — ${p.revenue.toFixed(2)} EUR</li>`)
    .join('')

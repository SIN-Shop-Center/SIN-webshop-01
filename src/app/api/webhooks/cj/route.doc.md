...
**Docs:** `src/app/api/webhooks/cj/route.ts`
...
- `src/lib/emails/send.ts` — shipping/delivery email dispatch
- `src/lib/fulfillment/submit-order.ts` — forwards paid orders to CJ
- `src/app/api/cron/fulfillment-retry/route.ts` — retry cron for failed forwards
- `tooling/scripts/supabase/setup-cj.sql` — `cj_order_id`, `tracking_number` columns

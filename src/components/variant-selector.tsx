...
export function VariantSelector({
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      try {
        await addToCartClient(product.id, quantity, selectedVariant?.cj_variant_id)
        setAdded(true)
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button
        ...
        data-state={outOfStock ? 'sold-out' : isPending ? 'pending' : added ? 'added' : 'idle'}
      >
        {outOfStock ? 'Ausverkauft' : isPending ? 'Wird hinzugefügt…' : added ? 'Hinzugefügt ✓' : 'In den Warenkorb'}
      </button>
      <span className="sr-only" role="status" aria-live="polite" data-testid="add-to-cart-status">

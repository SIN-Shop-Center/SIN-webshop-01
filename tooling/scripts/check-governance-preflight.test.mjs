...
test('governance preflight allows nested markdown drift outside AGENTS', async () => {
  ...
  await fs.writeFile(path.join(repoDir, 'platform', 'workers', 'n8n', 'README.md'), '# changed\n', 'utf8')
...
test('governance preflight still fails on non-markdown doc drift outside AGENTS', async () => {
  ...
  await fs.writeFile(path.join(repoDir, 'platform', 'workers', 'n8n', 'notes.txt'), 'changed\n', 'utf8')
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.match(error.stderr, /platform\/workers\/n8n\/notes\.txt/)
      return true
    },
  )
})
...
async function createRepoFixture() {
  ...
  await fs.mkdir(path.join(dir, 'platform', 'workers', 'n8n'), { recursive: true })
...
function buildScriptEnv(overrides = {}) {
  return {
    ...process.env,
    // Fixture executions must exercise the live preflight path even when the
    // parent test runner itself is running in CI mode.
    CI: 'false',
    NLM_QUERY_RETRIES: '0',

 ...
 * Usage:
 *   node tooling/scripts/cj/import-products.mjs --keyword "wireless earbuds" --limit 20
 *   node tooling/scripts/cj/import-products.mjs --pid <CJ_PRODUCT_ID>
 *
 * Sicherheit: EU-Lagerbestand ist standardmäßig Pflicht. Nur mit dem expliziten
 * Flag --allow-non-eu darf davon abgewichen werden. Neue Produkte bleiben bis
 * zum Quality-/Compliance-Gate inaktiv; --activate ist eine bewusste Ausnahme.
 *
...
const REQUIRE_EU_STOCK = !args.includes('--allow-non-eu')
const ACTIVATE_IMMEDIATELY = args.includes('--activate')
...
async function getEuStock(vid) {
  try {
    const data = await cj('/product/stock/queryByVid', { vid })
    const list = Array.isArray(data) ? data : []
    return list.reduce((total, warehouse) => {
      const code = String(warehouse.countryCode || warehouse.areaEn || '').toUpperCase()
      if (!EU_WAREHOUSES.has(code)) return total
      return total + Math.max(0, Number(warehouse.storageNum ?? warehouse.num ?? 0))
    }, 0)
  } catch (error) {
    console.warn(`EU-Bestand für Variante ${vid} nicht prüfbar: ${error.message}`)
    return 0
  }
}
...
/**
 * CJ-Variante → JSONB-Format, das parseVariants() in src/lib/queries.ts liest:
 * { cj_variant_id, sku, name, price, stock, image_url }
 */
function mapVariant(v, verifiedStock) {
  ...
  return {
    ...
    stock: Math.max(0, Number(verifiedStock ?? v.variantStock ?? 0)),
...
async function importProduct(pid) {
  ...
  for (const variant of cjVariants) {
    const euStock = REQUIRE_EU_STOCK ? await getEuStock(variant.vid) : null
    if (!REQUIRE_EU_STOCK || Number(euStock) > 0) {
      verifiedVariants.push({ raw: variant, stock: euStock })
    }
    if (REQUIRE_EU_STOCK) await new Promise((resolve) => setTimeout(resolve, 350))
  }

  if (!verifiedVariants.length) {
    console.warn(`Übersprungen (kein nachgewiesener EU-Lagerbestand): ${detail.productNameEn}`)
    return
  }

  const firstVariant = verifiedVariants[0].raw
  const cost = Number(firstVariant.variantSellPrice ?? detail.sellPrice)
  const variants = verifiedVariants.map(({ raw, stock }) => mapVariant(raw, stock))
  const images = collectImages(detail, verifiedVariants.map(({ raw }) => raw))
  ...
  const row = {
    ...
    pipeline_state: ACTIVATE_IMMEDIATELY ? 'published' : 'sourced',
    approval_state: ACTIVATE_IMMEDIATELY ? 'override' : 'review_required',
    data_quality_score: 0,
    creative_status: 'missing',
    risk_level: 'unknown',
    publish_blockers: ACTIVATE_IMMEDIATELY
      ? []
      : ['Enrichment, Creative, GPSR und Freigabe ausstehend'],
    ...
    metadata: {
      pipeline_state: ACTIVATE_IMMEDIATELY ? 'published' : 'sourced',
      approval_state: ACTIVATE_IMMEDIATELY ? 'override' : 'review_required',
      eu_stock_verified: REQUIRE_EU_STOCK,
...
if (pid) {
  ...
  if (!keyword) {
    console.error(
      'Usage: --keyword "suchbegriff" [--limit 10] [--allow-non-eu] [--activate] ODER --pid <id>',
    )
    process.exit(1)
  }
  const data = await cj('/product/list', {
    ...
    countryCode: REQUIRE_EU_STOCK ? 'DE' : 'CN',
  })

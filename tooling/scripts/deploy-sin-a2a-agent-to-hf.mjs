...
const registryFile = path.join(projectRoot, 'platform', 'agents', 'sin-a2a', 'registry.json')
...
async function buildSpmSpaceBundle({ agent, bundleRoot }) {
  ...
  await copyTree(path.join(projectRoot, 'platform', 'workers', 'spm-a2a'), path.join(bundleRoot, 'workers', 'spm-a2a'))
  await copyTree(path.join(projectRoot, 'platform', 'workers', 'spm-secret-sync-mcp'), path.join(bundleRoot, 'workers', 'spm-secret-sync-mcp'))

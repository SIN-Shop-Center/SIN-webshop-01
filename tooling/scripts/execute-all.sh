...
echo "   → Kopiere den Inhalt von: tooling/scripts/supabase/setup-title-de.sql"
echo "   → Klicke 'Run'"
echo ""
echo "   Option B: psql (falls du direkten DB-Zugang hast)"
echo "   → psql \"$DATABASE_URL\" -f tooling/scripts/supabase/setup-title-de.sql"
echo ""
...
node tooling/scripts/cj/backfill-product-data.mjs --dry-run 2>&1 | head -20 || true
echo ""
read -p "   🟡 Trockenlauf OK? Dann ENTER für echte Ausführung (oder Ctrl+C) ..."
echo ""
node tooling/scripts/cj/backfill-product-data.mjs 2>&1 | tail -30
echo ""
...
if [ -n "${AI_GATEWAY_API_KEY:-}" ]; then
  node tooling/scripts/cj/translate-products.mjs 2>&1 | tail -30
else
...
if [ -n "${CRON_SECRET:-}" ]; then
  export CRON_SECRET
  node tooling/scripts/cj/trigger-reviews.mjs 2>&1
else

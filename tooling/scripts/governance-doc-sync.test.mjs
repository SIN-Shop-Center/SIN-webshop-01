...
test('browser governance dry-run emits the V2 marker and both governance terms', async () => {
  const output = await runNodeScript('tooling/scripts/push-browser-governance-to-gdoc.mjs', ['--dry-run'])
...
test('storefront governance dry-run emits the storefront marker and caller timestamp', async () => {
  ...
  const output = await runNodeScript('tooling/scripts/push-storefront-governance-to-gdoc.mjs', [
    '--dry-run',

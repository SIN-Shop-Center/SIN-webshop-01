...
export function resolveWebProductionEnv(env = process.env) {
  const fallbackUrl = String(env.WEB_PRODUCTION_ENV_FALLBACK_URL || '').trim()
  const rawSiteUrl = String(env.SITE_URL || fallbackUrl).trim()
  const rawPublicAppUrl = String(env.NEXT_PUBLIC_APP_URL || fallbackUrl).trim()

...
const describeDatabase = destructiveTestsEnabled ? describe : describe.skip
...
describeDatabase('Inventory Race-Condition (#37)', () => {
  it('1. reserve_stock(qty=1) auf Produkt mit stock=1: 1× success, 1× P0001', async () => {
    ...
    try {
      ...
      // Supabase JS resolves RPC calls as { data, error }; it does not expose
      // a separate `success` property. One reservation must succeed and the
      // competing transaction must fail with the database exception code.
      const ok = results.filter((result) => !result.error)
      const exhausted = results.filter((result) => result.error?.code === 'P0001')

...
def ssh_cmd(cmd):
    return subprocess.run(
        ["ssh", "-i", SSH_KEY, VM, cmd], capture_output=True, text=True, timeout=30
    )
...
def main():
    r = ssh_cmd(
        "docker exec supabase-db psql -U supabase_admin -d postgres -t -A -c \"SELECT (id || '|' || sku || '|' || COALESCE(metadata->>'cj_pid','')) FROM shop.products WHERE metadata->>'cj_pid' IS NOT NULL AND metadata->>'cj_vid' IS NULL\""
    )
    rows = [
        line.split("|") for line in r.stdout.strip().split("\n") if line and "|" in line
    ]
    ...
    subprocess.run(
        ["scp", "-i", SSH_KEY, tmp, f"{VM}:/tmp/backfill_vids.sql"],
        capture_output=True,
        timeout=15,
    )
    r2 = ssh_cmd(
        "docker cp /tmp/backfill_vids.sql supabase-db:/tmp/backfill_vids.sql && docker exec supabase-db psql -U supabase_admin -d postgres -f /tmp/backfill_vids.sql"
    )
    print(r2.stdout)
    if r2.returncode == 0:

...
def cj_product_list_v2(
    ...
    if start_price > 0:
        params["startSellPrice"] = start_price
    if end_price > 0:
        params["endSellPrice"] = end_price
    if free_shipping >= 0:
        params["addMarkStatus"] = free_shipping
    if order_by > 0:
        params["orderBy"] = order_by
    if product_flag >= 0:
        params["productFlag"] = product_flag
...
def cj_product_list(
    ...
    if category_id:
        params["categoryId"] = category_id
    if product_name_en:
        params["productNameEn"] = product_name_en
    if country_code:
        params["countryCode"] = country_code
    if min_price > 0:
...
def cj_product_reviews(pid: str, page_num: int = 1, page_size: int = 20) -> dict:
    ...
    return _api_call(
        "GET",
        "/product/review/list",
        params={"pid": pid, "pageNum": page_num, "pageSize": page_size},
    )
...
def cj_product_sourcing_create(name: str, image: str, link: str) -> dict:
    ...
    return _api_call(
        ...
        body={"productName": name, "productImage": image, "productLink": link},
    )
...
def cj_product_connect(pid: str, sku: str, store_sku: str) -> dict:
    ...
    return _api_call(
        ...
        body={"productId": pid, "sku": sku, "storeSku": store_sku},
    )
...
@mcp.tool()
def cj_private_inventory_spu_page(
    keyword: str = "",
    page_num: int = 1,
    page_size: int = 20,
    available_stock: bool = False,
) -> dict:
...
@mcp.tool()
def cj_private_inventory_sku_detail(
    sku: str = "", page_num: int = 1, page_size: int = 20, available_stock: bool = False
) -> dict:
...
@mcp.tool()
def cj_private_inventory_sku_flow(
    sku: str = "", page_num: int = 1, page_size: int = 20
) -> dict:
...
def cj_order_create_v2(
    ...
    remark: str = "",
    ioss_type: int = 3,
    ioss_number: str = "CJ-IOSS",
    platform: str = "api",
    shop_logistics_type: int = 2,
    order_flow: int = 1,
) -> dict:
    ...
    body = {
        ...
        "platform": platform,
        "shopLogisticsType": shop_logistics_type,
        "orderFlow": order_flow,
    }
    if shipping_zip:
        body["shippingZip"] = shipping_zip
    if shipping_phone:
        body["shippingPhone"] = shipping_phone
    if email:
        body["email"] = email
    if remark:
        body["remark"] = remark
...
def cj_order_add_cart(order_ids: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/order/addCart", body={"cjOrderIdList": order_ids.split(",")}
    )
...
def cj_order_add_cart_confirm(order_id: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/order/addCartConfirm", body={"orderId": order_id}
    )
...
def cj_order_detail(order_id: str) -> dict:
    ...
    return _api_call(
        "GET", "/shopping/order/getOrderDetail", params={"orderId": order_id}
    )
...
def cj_order_change_warehouse(order_id: str, storage_id: str) -> dict:
    ...
    return _api_call(
        ...
        body={"orderId": order_id, "storageId": storage_id},
    )
...
def cj_balance_pay_v2(order_ids: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/pay/payBalanceV2", body={"orderIdList": order_ids.split(",")}
    )
...
def cj_shipping_update_pod(order_id: str, image_urls: str) -> dict:
    ...
    return _api_call(
        ...
        body={"orderId": order_id, "imageUrlList": image_urls.split(",")},
    )
...
def cj_cogs_query(order_ids: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/cogs/query", body={"orderIdList": order_ids.split(",")}
    )
...
def cj_merge_auto_progress(batch_id: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/order/autoMergeQueryProgress", body={"batchId": batch_id}
    )
...
def cj_merge_auto_result(batch_id: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/order/autoMergeQueryResult", body={"batchId": batch_id}
    )
...
def cj_merge_submit_batch(order_ids: str) -> dict:
    ...
    return _api_call(
        "POST",
        "/shopping/order/submitMergeBatch",
        body={"orderIdList": order_ids.split(",")},
    )
...
def cj_merge_submit_progress(batch_id: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/order/submitMergeProgress", body={"batchId": batch_id}
    )
...
def cj_merge_submit_result(batch_id: str) -> dict:
    ...
    return _api_call(
        "POST", "/shopping/order/submitMergeResult", body={"batchId": batch_id}
    )
...
def cj_freight_calculate_tip(from_country: str, to_country: str, products: str) -> dict:
    ...
    return _api_call(
        ...
        body={
            "reqDTOS": [
                {
                    "srcAreaCode": from_country,
                    "destAreaCode": to_country,
...
def cj_freight_supplier_self(from_country: str, to_country: str, products: str) -> dict:
    ...
    return _api_call(
        "POST",
        "/logistic/supplierSelfShipmentFreightCalculate",
        body={
            "startCountryCode": from_country,
            "endCountryCode": to_country,
            "products": json.loads(products),
...
def cj_dispute_confirm(order_id: str, items: str) -> dict:
    ...
    return _api_call(
        ...
        body={"orderId": order_id, "productInfoList": json.loads(items)},
    )


@mcp.tool()
def cj_dispute_create(
    order_id: str,
    business_dispute_id: str,
    reason_id: int,
    expect_type: int,
    refund_type: int,
    message: str,
    ...
    body = {
        "orderId": order_id,
        "businessDisputeId": business_dispute_id,
        "disputeReasonId": reason_id,
...
def cj_dispute_cancel(order_id: str, dispute_id: str) -> dict:
    ...
    return _api_call(
        "POST", "/disputes/cancel", body={"orderId": order_id, "disputeId": dispute_id}
    )
...
def cj_dispute_detail(dispute_id: str) -> dict:
    ...
    return _api_call(
        "GET", "/disputes/getDisputeDetail", params={"disputeId": dispute_id}
    )
...
def cj_webhook_set(
    ...
    for topic, enabled in [
        ...
        ("stock", enable_stock),
        ("order", enable_order),
        ("logistics", enable_logistics),
    ]:
        body[topic] = {
            "type": "ENABLE" if enabled else "CANCEL",
...
def cj_webhook_subscribe(product_ids: str = "", subscribe_all: bool = False) -> dict:
    """Subscribe to docs/product/variant/stock webhook notifications for specific products or all.
...
def cj_webhook_subscribed_list(page_num: int = 1, page_size: int = 20) -> dict:
    ...
    return _api_call(
        "GET",
        "/webhook/product/subscribe/list",
        params={"pageNum": page_num, "pageSize": page_size},
    )
...
def cj_shop_save_product(
    ...
    if price_min > 0:
        body["priceCurrency"] = price_currency
...
def cj_shop_save_variants(variants: str) -> dict:
    ...
    return _api_call(
        "POST",
        "/store/product/saveVariantBatch",
        body={"variants": json.loads(variants)},
    )
